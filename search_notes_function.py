...
def search_notes(notes, input_crit, keyword = None, status = None):
...
def input_crit_(notes, input_crit):
    print()
    print('=================== Поиск заметок ==================')
    ...
    while True:
        ...
        if not input_crit: # если пустой ввод
            ...
            found_notes = search_notes(notes, input_crit, keyword = input_word)
            ...
            while input_stat != 'новая' and  input_stat != 'в процессе' and input_stat != 'выполнено':
                ...
                input_stat = input('Введите статус (новая,в процессе,выполнено): ').strip().lower()
            found_notes = search_notes(notes, input_crit, status = input_stat)
        elif input_crit == 3:   # отправляем в функцию поиска заметок оба критерия
            input_word = input('Введите ключевое слово: ').strip().lower()
            input_stat = input('Введите статус (новая,в процессе,выполнено): ').strip().lower()
            found_notes = search_notes(notes, input_crit, keyword=input_word, status=input_stat)
